...
public static class LogActivityContext
{
    ...
    public static ValueTask LoggingMediatorRequest(
        this ILogger logger,
        string prefix,
        ...
        object? request = default)
    {
        ...
        // TODO: disable this Trace. Devops guy said because this make memory leak
        //using Activity? activity = GetActivityContext(tracePrefix);
        //if (activity is null)
        //{
    ...
    public static void LogInformation<TType>(
        this ILogger logger,
        string template,
        params object?[] objects) where TType : class
    {
        logger.LogInformation(typeof(TType).Name, template, objects);
    }
    ...
    {
        using (LogContext.PushProperty(Constants.Serilog.TraceId, GetTraceId()))
        using (LogContext.PushProperty("Prefix", prefix))
        {
            logger.Information(template, objects);
        }
    }
    ...
    public static ValueTask LoggingMediatorResponse<TType>(
        this ILogger logger,
        object response,
        ActivityStatusCode statusCode = ActivityStatusCode.Ok) where TType : class
    {
        logger.LoggingMediatorResponse(typeof(TType).Name, response, statusCode);
        return ValueTask.CompletedTask;
    }

    public static ValueTask LoggingMediatorResponse(
        this ILogger logger,
        string prefix,
        object response,
        ActivityStatusCode statusCode = ActivityStatusCode.Ok)
    ...
    public static void LogWarning<TType>(
       this ILogger logger,
       string template,
       params object?[] objects) where TType : class
    {
        logger.LogWarning(typeof(TType).Name, template, objects);
    }
       ...
       params object?[] objects)
    {
        ...
        using (LogContext.PushProperty("Prefix", prefix))
        {
            logger.Warning(template, prefix, objects);
        }
    }
